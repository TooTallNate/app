...
import {
  QueryLivestockAdjustmentArgs,
  LivestockAdjustment
} from "../../common/graphql";
import { client, testUnauthenticated, mockUser } from "../../../test/utils";
import { JobFactory } from "../../../test/builders";
import LivestockAdjustmentModel from "../models/LivestockAdjustment";
import { LivestockAdjustmentFactory } from "../../../test/builders";

function query(variables: QueryLivestockAdjustmentArgs) {
  return client.request<LivestockAdjustment>(
...
test("returns from from the database", async () => {
  const { job } = await mockTestData();
  const doc = await LivestockAdjustmentModel.create(
    LivestockAdjustmentFactory.build({
      job: job.No,
